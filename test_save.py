...
def calculate_asd(pred, gt):
    """
    Calculate Average Surface Distance (ASD) between predicted and ground truth masks.
    """
    ...
    # If no borders are found, return 0
    if len(pred_border_indices) == 0 or len(gt_border_indices) == 0:
        return 0.0

    # Compute distances between border points
...
def calculate_metric_percase(pred, gt):
    """
    Evaluate Dice similarity for binary segmentation masks.
    """
    pred[pred > 0] = 1
    gt[gt > 0] = 1
...
if __name__ == "__main__":
    ...
    # Define paths for input, output, and visualization
    ...
    for test_img_name in test_img_list:
        ...
        # Load the image, prediction, and ground truth based on file type
        if file_ext in ['png']:
        ...
        # Initialize visualization with the original image
        each_metric = []
        
        img_with_GT = img.copy()
        # Compute metrics and visualize segmentation results for each class
    ...
    # Convert metric list to numpy array and calculate overall statistics
