...
def calculate_dice(pred, gt):
    # Used to calculate the DICE similarity factor
    intersection = np.logical_and(pred, gt).sum()
    union = pred.sum() + gt.sum()

    # If both are empty sets, dice should be 1
    if union == 0:
        return 1.0
...
def download_model(url, destination):
    # Download model weights from a specified URL
...
class DynamicDataset(data.Dataset):
    # Custom dataset class for dynamic loading and preprocessing
    ...
    def __getitem__(self, item):
        ...
        if file_end in ['png']:
            ...
            if npimg.mode == 'L':  # 'L' mode represents a grayscale image
                npimg = np.array(npimg) / 255.0  # Remain single-channel
                npimg = np.expand_dims(npimg, axis=0)  # Add a dimension as a channel
            else:
        ...
        if self.size is not None:
            ...
            # Adapt size to match model requirements
...
def format_progress_message(iteration, total):
    # Format progress messages for training output
...
def train(batch_size=4, max_epochs=200, base_lr=0.01, seed=1234, n_gpu=1, model_name='swinunet'):
    ...
    # Set paths for output and datasets
    ...
    # Initialize logger, loss functions, optimizer
